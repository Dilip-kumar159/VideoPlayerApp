...
import com.dililp.myvideo.databinding.DetailsViewBinding
import com.dililp.myvideo.databinding.RenameFieldBinding
import com.dililp.myvideo.databinding.VideoMoreFeaturesBinding
import com.dililp.myvideo.databinding.VideoViewBinding
...
class VideoAdapter(private val context: Context, private var videoList: ArrayList<Video>, private val isFolder : Boolean = false )
    ...
    override fun onBindViewHolder(holder: MyHolder, @SuppressLint("RecyclerView") position: Int) {
        ...
        Glide.with(context)
            ...
            .apply(RequestOptions().placeholder(R.mipmap.ic_my_video_player).centerCrop())
            .into(holder.image)
    ...
    // for requesting android 11 and above for storage permission
...
//    }
    ...
    private fun requestDeleteR(position: Int){
        ...
        if(Build.VERSION.SDK_INT >= Build.VERSION_CODES.R){
            // for requesting permission to delete a video
            ...
            builder.setTitle("Delete Video?")
                ...
                .setPositiveButton("Yes"){ self, _ ->
                    if(file.exists() && file.delete()){
                        ...
                        updateDeletedUI(position = position)
                    }
                    self.dismiss()
                }
    ...
    @SuppressLint("NotifyDataSetChanged")
    private fun updateDeletedUI(position: Int){
        when{
            MainActivity.search -> {
...
//                notifyItemChanged(position)
    ...
    fun onResult(requestCode : Int, resultCode : Int){
        when(requestCode){
            123 -> {
                if(resultCode == Activity.RESULT_OK) updateDeletedUI(newPosition)
            }
            124 -> {
